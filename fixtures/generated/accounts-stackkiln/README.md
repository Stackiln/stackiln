...
Local contact messages are written to `apps/web/.local-mailbox`. In a container test environment, the default is `/tmp/stackkiln-mailbox`; set `LOCAL_MAILBOX_DIR` to persist them elsewhere. Production email requires Resend credentials.
...
This product was generated by StackKiln. Its source is independent of the generator and may be used, modified, and relicensed subject to the notices in `STACKKILN-LICENSE.md`.
