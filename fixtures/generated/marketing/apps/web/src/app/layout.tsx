...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="en"><body><a className="skip-link" href="#main">Skip to content</a><header className="site-header"><Link className="wordmark" href="/">{config.product.name}</Link><MainNavigation /></header><main id="main">{children}</main><footer className="site-footer"><span>{config.product.name}</span><LegalNavigation /></footer><EnabledComponents /></body></html>;
}
