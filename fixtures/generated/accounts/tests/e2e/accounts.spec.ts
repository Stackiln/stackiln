...
async function messageUrl(address: string, subject: string): Promise<string> {
  let files: string[];
  try { files = await readdir(mailbox); } catch { return ""; }
  for (const file of files) {
    const message = JSON.parse(await readFile(join(mailbox, file), "utf8")) as { to: string; subject: string; text: string };
...
test("account verification, recovery, sessions, export and deletion", async ({ page, browser }) => {
  ...
  await expect.poll(() => messageUrl(address, "Verify your email")).not.toBe("");
  ...
  await expect(page.getByRole("status")).toContainText("Check your current email");
  await expect.poll(() => messageUrl(address, "Approve email change")).not.toBe("");
  await page.goto(await messageUrl(address, "Approve email change"));
  await expect.poll(() => messageUrl(nextAddress, "Verify your email")).not.toBe("");
  ...
  await expect(page.getByRole("status")).toContainText("If that address has an account", { timeout: 15_000 });
  await expect.poll(() => messageUrl(address, "Reset your password")).not.toBe("");
  ...
  try {
    ...
    expect((await remote.request.get("/api/account/export")).status()).toBe(401);
    ...
    await page.reload();
    const remoteSession = page.locator(".session-list li").filter({ hasText: "Remote-Test-Session" });
  ...
  await expect(page.getByRole("status")).toContainText("Check your email to confirm account deletion");
  await expect.poll(() => messageUrl(address, "Confirm account deletion")).not.toBe("");
