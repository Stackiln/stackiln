...
The generated home page is composed from product-owned block components under `apps/web/src/blocks`. Its resolved order and source catalogue are documented in `docs/blocks.md`; edit those components normally after generation.

Local contact messages are written to `apps/web/.local-mailbox`. In a container test environment, the default is `/tmp/stackiln-mailbox`; set `LOCAL_MAILBOX_DIR` to persist them elsewhere. Production email requires Resend credentials.
