...
export async function POST(request: Request) {
  const documentRequest = request.headers.get("accept")?.includes("text/html") ?? false;
  const error = (message: string, status: number) => documentRequest
  ...
  if (request.headers.get("origin") !== trustedOrigin) return error("Invalid request origin.", 403);
  const input = documentRequest ? Object.fromEntries(await request.formData()) : await request.json().catch(() => null);
  ...
  try {
    ...
    return documentRequest ? Response.redirect(new URL("/contact/thanks", request.url), 303) : Response.json({ ok: true });
  } catch { return error("Message could not be sent. Please retry.", 503); }
}
