...
async function run(args: string[]): Promise<number> {
  ...
  if (command === "create") {
    ...
    process.stdout.write(rest.includes("--json") ? `${JSON.stringify({ directory: resolve(directory), state }, null, 2)}\n` : `Created ${resolve(directory)}\n${formatPlan(plan)}\n`);
    return 0;
  }
  if (command === "inspect" || command === "doctor" || command === "context") {
    const root = resolve(rest[0] && !rest[0].startsWith("--") ? rest[0] : ".");
    const state = await readState(root);
    const config = JSON.parse(await readFile(join(root, "factory.config.json"), "utf8")) as { deployment: { target: string } };
    const changed = await validateProduct(root, state);
    if (command === "inspect") {
      const result = { preset: state.preset, modules: state.installed, factoryVersion: state.factoryVersion, deployment: config.deployment.target, changedManagedFiles: changed };
      process.stdout.write(rest.includes("--json") ? `${JSON.stringify(result, null, 2)}\n` : `${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    if (command === "doctor") {
      const checks = { state: "ok", managedFiles: changed.length ? `changed: ${changed.join(", ")}` : "ok", databaseUrl: process.env.DATABASE_URL ? "configured" : "missing", appUrl: process.env.APP_URL ? "configured" : "missing" };
    ...
    if (rest.includes("--write")) await writeFile(join(root, "AGENT_CONTEXT.md"), snapshot);
    process.stdout.write(snapshot);
    return 0;
  }
  if (command === "verify") {
    const result = spawnSync(process.platform === "win32" ? "pnpm.cmd" : "pnpm", ["verify"], { cwd: factoryRoot, stdio: "inherit", shell: process.platform === "win32" });
    return result.status ?? 1;
  }
  throw new Error(`Unknown or unimplemented command: ${command}. Run --help for available commands.`);
}
run(process.argv.slice(2)).then(code => { process.exitCode = code; }).catch(error => { console.error(error instanceof Error ? error.message : error); process.exitCode = 2; });
